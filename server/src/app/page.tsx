...
export default function Page() {
  ...
  const refreshSuggestions = (currentLevel?: string) => {
    const list = levelWordsMap[(currentLevel || level) as keyof typeof levelWordsMap];
  ...
  const handleStart = async (wordToUse?: string) => {
    ...
    try {
      ...
      setCurrentStory({ word, story: "Error generating content. Please check your connection and API keys.", drills: [] });
    } finally {
      setLoading(false);
      setInputWord('');
    }
  ...
  const handleGuestLogin = () => {
    setUser({ name: 'Guest Explorer', picture: '👤' });
    setCurrentView('home');
  };
    ...
    <div className={`min-h-screen ${activeTheme.bg} ${activeTheme.text} ${activeTheme.font} transition-colors duration-700 overflow-hidden flex flex-col relative`}>
      ...
      <main className="flex-1 px-6 pb-32 overflow-y-auto z-10 relative no-scrollbar">
        <AnimatePresence mode="wait">
          {currentView === 'splash' && (
            <motion.div key="splash" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="h-full flex flex-col items-center justify-center space-y-12 py-20">
                <div className="relative">
                    <motion.div animate={{ rotate: 360 }} transition={{ duration: 10, repeat: Infinity, ease: "linear" }} className={`w-32 h-32 rounded-full border-4 border-dashed ${activeTheme.subText} opacity-20`} />
                    <Zap size={64} className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 ${activeTheme.subText}`} />
                </div>
                <div className="text-center space-y-4">
                    <h1 className="text-5xl font-black tracking-tighter uppercase italic">VocabVortex</h1>
                    <p className="opacity-40 font-bold uppercase tracking-[0.3em] text-xs">Master the English Universe</p>
                </div>
                <button 
                    onClick={handleGuestLogin}
                    id="guest-login-btn"
                    className={`${activeTheme.accent} text-white px-12 py-6 rounded-[32px] font-black text-lg shadow-2xl flex items-center gap-4 active:scale-95 transition-all`}
                >
                    ENTER VORTEX <ChevronRight size={24} />
                </button>
            </motion.div>
          )}
            ...
            <motion.div key="home" initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0, scale: 0.95 }} className="space-y-10 py-6">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {levels.map(lvl => (
                      <button 
                        ...
                        className={`px-3 py-2 rounded-xl text-[10px] font-black border transition-all level-btn ${level === lvl ? `${activeTheme.accent} text-white border-transparent shadow-2xl` : 'border-current opacity-20 hover:opacity-100'}`}
                      >
                        {lvl}
                      </button>
                    ))}
                    ...
                    <div className="flex flex-wrap gap-2">
                      {suggestions.map((w, idx) => (
                        <motion.button
                          ...
                          className="bg-white/5 border border-white/5 px-4 py-2 rounded-2xl text-xs font-bold hover:bg-white/10 active:scale-95 transition-all suggestion-word"
                        >
                          {w}
                        </motion.button>
                      ))}
              ...
              <div className="grid grid-cols-2 gap-4">
                <div className={`${activeTheme.card} p-7 rounded-[40px] flex flex-col justify-between h-36`}>
                   ...
                   <div className="text-5xl font-black tracking-tighter italic" id="queue-count">{trainerQueue.length}</div>
                </div>
                <div 
          ...
          {currentView === 'reading' && (
            <motion.div key="reading" initial={{ opacity: 0, scale: 1.1 }} animate={{ opacity: 1, scale: 1 }} className="h-full flex flex-col gap-6">
              {loading ? (
                ...
                <>
                  <div className={`${activeTheme.card} p-10 rounded-[56px] flex-1 flex flex-col overflow-y-auto no-scrollbar border-white/5`} id="reading-pane">
                    ...
                    <div className="space-y-16">
                      <div className="font-serif leading-relaxed text-2xl">
                        <p className="opacity-90 leading-[2] italic text-center" id="story-text">
                          {currentStory?.story?.split(' ').map((w: string, i: number) => (
                      ...
                      <div className="pt-12 border-t border-white/5">
                        ...
                        <div className="space-y-5" id="drills-list">
                          {currentStory?.drills?.map((drill: any, i: number) => (
                            <motion.div 
                              key={i} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.1 }}
                              className="group p-6 rounded-[32px] bg-white/5 border border-white/5 hover:border-white/20 transition-all relative drill-item"
                            >
                               <div className="flex flex-wrap items-center leading-relaxed">
                                  {drill.sentence.split(' ').map((w: string, idx: number) => (
                               ...
                               <button 
                                ...
                                className="absolute -top-3 -right-3 w-10 h-10 rounded-full bg-white/10 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-all border border-white/10 backdrop-blur-xl drill-info-btn"
                               >
                      ...
                      <div className="pt-12 pb-6 text-center border-t border-white/5">
                         ...
                         <p className="text-4xl font-black tracking-tight bg-gradient-to-b from-white to-white/20 bg-clip-text text-transparent" id="bengali-definition">{currentStory?.bengaliDefinition}</p>
                      </div>
                    </div>
                  </div>
          ...
          {currentView === 'trainer' && (
            <motion.div key="trainer" initial={{ x: 300 }} animate={{ x: 0 }} exit={{ x: 300 }} className="space-y-8">
              ...
              <div className="space-y-4" id="trainer-queue-list">
                {trainerQueue.length === 0 ? (
                  ...
                  trainerQueue.map((item, idx) => (
                    <motion.div key={item.id} layout className={`${activeTheme.card} p-6 rounded-[32px] flex justify-between items-center border-white/5 trainer-item`}>
                       <div className="flex items-center gap-5">
                       ...
                       <button onClick={() => setTrainerQueue(q => q.filter(x => x.id !== item.id))} className="text-red-500/20 hover:text-red-500 transition-colors p-3 remove-word-btn"><Trash2 size={24}/></button>
                    </motion.div>
                  ))
                )}
              </div>
      ...
      <AnimatePresence>
        {selectedWord && (
          <div className="fixed inset-0 z-50 flex items-center justify-center p-8 bg-black/90 backdrop-blur-md" id="word-insight-modal">
