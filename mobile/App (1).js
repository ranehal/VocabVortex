  ...
  return (
    ...
    >
      <View style={[styles.readFlowTopCard, { backgroundColor: '#f5f0e8' }]}>
        <View style={styles.readFlowHeaderRow}>
          <View>
            ...
            <Text style={styles.readFlowSubtitle}>
              English chunk reading + Bengali reveal practice
            </Text>
          </View>
        ...
        <View style={styles.readFlowActions}>
          <TouchableOpacity onPress={loadText} style={styles.readFlowPrimaryBtn}>
            <Text style={styles.readFlowPrimaryBtnText}>LOAD</Text>
          </TouchableOpacity>
      ...
      <View style={[styles.readFlowPanel, { backgroundColor: '#faf7f2' }]}>
        ...
        {chunks.length === 0 ? (
          <Text style={styles.readFlowEmpty}>
            Paste text using this format: English phrase (বাংলা অর্থ). Then tap Load.
          </Text>
        ) : (
      ...
      <View style={[styles.readFlowPanel, { backgroundColor: '#faf7f2' }]}>
        ...
        {queue.length === 0 ? (
          <Text style={styles.readFlowEmpty}>
            পড়তে পড়তে কোনো phrase-এ tap করলে সেটা practice queue-তে যোগ হবে।
          </Text>
        ) : (
          queue.map((item, i) => (
            <TouchableOpacity
